...
@section('content')
    <div class="row">
        ...
        {{-- ACCESS TOKEN FORM --}}
        <div class="col-lg-8">
            <form class="form" action="{{ route('backpack.account.access-token') }}" method="post">
                {!! csrf_field() !!}
                <div class="card padding-10">
                    <div class="card-header">
                        API Access Token
                    ...
                    <div class="card-body backpack-profile-form bold-labels">
                        <div class="row">
                            <div class="col-md-8 form-group">
                                ...
                                <label class="required">{{ $label }}</label>
                                @if($access_token)
                                <input autocomplete="token" disabled class="form-control" type="text" name="{{ $field }}" id="{{ $field }}" value="{{ $access_token }}">
                                @else
                                    <p>Create an Access Token By Clicking Generate Token Below.</p>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        <button type="submit" class="btn btn-success"><i class="la la-save"></i> @if(!$access_token) Generate Token @else Refresh Token @endif </button>
                    </div>
                </div>
            </form>
        </div>
